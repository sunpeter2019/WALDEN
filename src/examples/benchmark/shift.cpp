...
int main(int argc, char* argv[]) {
  ...
  */
  ...
  auto values = new std::tuple<KEY_TYPE, PAYLOAD_TYPE, WEIGHT_TYPE>[total_num_keys];  //�޸Ĵ洢���ݽṹ��
  ...
  while (true) {
    ...
    if(dis_shift == true){
        ...
        gmm_para2 = gen_fit(keys2, count2 );
        ...
        if(dis_shift_mark>=2){
            ...
            rebuild_time = std::chrono::duration_cast<std::chrono::nanoseconds> (rebuild_end_time - rebuild_start_time).count();                       
            std::cout << "rebuild_time " << rebuild_time << std::endl;
        }
